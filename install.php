...
?>
you are already had installed Ajax IM, if you want to reinstall it ,please delete 'install.lock' file.
...
<?PHP
	...
	function library_array($path, $exclude, $recursive = false) {
		...
		foreach($files as $f) {
			...
			preg_match('/^<'.'\?php[^\/]*\/\*(.+?)\*\//ism', $library_src, $library_header);
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		$configphp = file_get_contents('config_default.php');
		...
		if(intval($_POST['needs_nodejs']) == 1) {
			...
			file_put_contents('js/im.load.js', $imloadjs);
		}

		file_put_contents('config.php', $configphp);
		CONFIG::setconf(array('IM_LIBRARY'), str_replace('.php', '', $_POST['server']));
		CONFIG::setconf(array('DB_ENGINE'), str_replace('.php', '', $_POST['db']));
		CONFIG::setconf(array('COOKIE', 'NAME'), $_POST['cookie']);
		CONFIG::setconf(array('COOKIE', 'PERIOD'), $_POST['cookie_length']);
		CONFIG::setconf(array('MYSQL', 'HOSTNAME'), $_POST['mysqlhost']);
		CONFIG::setconf(array('MYSQL', 'DATABASE'), $_POST['mysqldb']);
		CONFIG::setconf(array('MYSQL', 'USERNAME'), $_POST['mysqluser']);
		CONFIG::setconf(array('MYSQL', 'PASSWORD'), $_POST['mysqlpass']);
		CONFIG::setconf(array('MYSQL', 'PREFIX__'), $_POST['mysqlprefix']);
		CONFIG::setconf(array('DB_ENGINE'), str_replace('.php', '', $_POST['db']));
		CONFIG::setconf(array('DB_ENGINE'), str_replace('.php', '', $_POST['db']));
		CONFIG::setconf(array('DB_ENGINE'), str_replace('.php', '', $_POST['db']));
		CONFIG::setconf(array('DB_ENGINE'), str_replace('.php', '', $_POST['db']));
			...
			<fieldset>
				...
				<ul>
					...
					<?php if(!$_POST['need_nodejs'] == 1) { ?>
						<li>The Node.js server host has been added to
							<code>js/im.load.js</code>; please confirm that the URL generated is correct.
						</li>
						<li>
							...
							<p>However, it is recommended that you move the
								<code>server</code> folder outside of the main installation directory before going live.
							</p>
						</li>
						<li>
							<p>Now that Ajax IM is installed, please see the
								<a href="http://ajaxim.com/documentation/basics/quick-start-nodejs/">Quick Start with Node.js</a> guide to complete set up.
							</p>
						</li>
					<?php } else { ?>
						<li>Since you did
							<strong>not</strong> setup Ajax IM with the Node.js server, it is recommended that you delete the
							<code>server</code> folder (containing the files <code>config.js</code> and
							<code>server.js</code>).
						</li>
						<li>
							<p>Now that Ajax IM is installed, please see the
								<a href="http://ajaxim.com/documentation/basics/quick-start/">Quick Start</a> guide to complete set up.
							</p>
						</li>
					<?php } ?>
				</ul>
	...
	<?php
		...
		<fieldset>
			...
			<p>
				...
				<?PHP is_file('config.php') || touch('config.php') ?>
			...
			<p>
				...
				<script type="text/javascript">
					...
					$(document).on('click', '#server', update_server_list);
					$(document).on('change', '#server', update_server_list);
				</script>
			</p>
			...
			<p>
				...
				<script type="text/javascript">
					...
					$(document).on('click', '#db', update_db_list);
					$(document).on('change', '#db', update_db_list);
					...
					$(document).on('click', '#db, #server', update_reqs);
					$(document).on('change', '#db, #server', update_reqs);
				</script>
			</p>
		...
		<fieldset id="nodejs-config">
			...
			<p>After configuration is complete, please see the
				<a href="http://ajaxim.com/installation">installation tutorial</a> to complete the standalone server setup.
			</p>
...
<p id="footer"><a href="http://ajaxim.com">Ajax IM</a> &copy; 2005&thinsp;&ndash;&thinsp;2010,
	<a href="http://unwieldy.net">Joshua Gross</a></p></p>
</body>
</html>